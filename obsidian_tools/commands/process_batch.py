...
Runs in-cluster. One invocation is one batch run — the agent handle goes down, the batch stream is
drained in strict FIFO, the handle comes back up. See `obsidian_tools/batch_processor/` for the
five obligations that shape it, and `chunk.py` for the wire contract it consumes.
"""
...
from obsidian_tools.batch_processor.agent_handle import AgentHandleError
...
# survives in the message. A failure here leaves the agent handle to the watchdog's lease, which is
# the whole reason the watchdog is a separate process.
_RUN_FAILURES = (AgentHandleError, BatchConsumerError, McpError)
