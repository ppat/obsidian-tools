...
7. **Observability** `[U]` — validation rejections, quarantine depth, inbox depth `[C1 threshold >20]`, lint findings by class, replica lag, MCP error rate, REST API liveness `[C2 Stage 5]`, batch-queue depth, dead-letter depth `[D]`.
...
| Vault worker | ingestor | read/search, patch, frontmatter, append, move/promote `[D]` |
...
```yaml
type: note            # required; note|source|entity|concept|project|person|task|decision|devlog|meeting|research|moc
title: "…"
...
Conventions, all `[C1]`: ISO dates everywhere; lowercase tags; one field = one type vault-wide; non-empty `type` on every note; **plural** `tags`/`aliases`/`cssclasses` per the 1.9.10 breaking change; declare property types on day one — now including `authority:` and `trigger:`, locked at Phase 1 alongside everything else (§7).

Overlays `[C1]`: strictness dials on two shared axes, not a different rule per domain (§3). Finance sits at the strict end of both — mandatory inline recency markers `(as of 2026-07, source.com)` and the tightest staleness cadence. Travel requires recency markers on perishable facts. Homelab is loosest. `refs:` supports a computed dependency graph enabling push-based staleness, vault-wide.
...
*Inject:* delete a curated note. Restore from Longhorn snapshot (P9). Then from git (P9). Then wedge the Electron process and confirm the probe restarts it `[C2 Stage 5]`.
...
**G3 — Where the bare git remote lives. Deliberately left for later.** The cluster is FluxCD GitOps `[U]`, so a cluster-reachable git remote necessarily exists `[D]` — whether personal vault content belongs there is policy, not a technical question. Alternative: a bare repo on the NAS `[U]`. No new product is required either way, and `git remote set-url` plus a re-push is cheap enough that nothing is gained by deciding now.
