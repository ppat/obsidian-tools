...
**Phase 2 — macOS and iOS read replicas.** Git committer + bare repo + pull-only clone; the `LAST_CHECKOUT` tag; the publish rsync (working tree only, no `.git`) into `iCloud Drive/Obsidian/<Vault Name>`; "Optimize Mac Storage" off; the shared exclude list. Open the same iCloud vault in Obsidian on the Mac and on the phone `[U]`.

**The durable capture store and the drift-reconciliation channel are deliberately *not* here** — they moved out of this phase. Phase 2 delivers the `PVC → human` direction only, which is the one the owner actually depends on: humans read on the devices, and edit there rarely. `human → PVC` is the exceptional path, so it lands with the agent write keys it needs rather than being built ahead of them (and a dispatch leg is itself a write, §7 Phase 5). Building it here would have meant either a classifier standing in for a promotion validator that does not exist yet, or a capture store accumulating edits nothing could dispatch. Narrowing the phase removes that dependency instead of working around it, and makes it shippable sooner.
...
*Inject:* delete a curated note on the volume, run the cycle → the deletion is recorded as history in git and the pull-only clone converges, proving the committer produces history rather than mutating content. Change a setting in `.obsidian/` on the device, run the cycle → the device's configuration survives untouched, proving the publish does not overwrite it. The three residual iCloud questions of §4 are answered here, by doing. The capture-before-publish injection that used to live here moves with the drift channel, to the phase that delivers it.
...
The durable capture store and the drift-reconciliation channel land here rather than at Phase 2 (§1.5 R2, §7 Phase 2): dispatch is a write through the agent handle, and this is the phase that opens one. The device-side detector stays dumb — it submits every path the comparison flags, including `.obsidian/`, and makes no judgement, so it can never silently drop a real edit. The server drops `.obsidian/` drift and classifies the rest. Keeping that policy server-side is deliberate: changing it later is a server change, with no device deployment and no fleet running divergent filter logic. The cost is that discarded `.obsidian/` changes transit the capture store before being dropped, which is cheap.
