...
| Check | Standing |
| --- | --- |
| **The NetworkPolicy packet test** — confirm the enforcement dependency on the nodes, then a throwaway default-deny-plus-one-allow test between labelled and unlabelled pods, the positive case proving the negative means something | **Parked by standing decision; reopen only on new evidence.** The one control with no fallback; config-level evidence is moderate-to-high, and an unenforced policy is invisible through the API — no passive signal exists (ADR-0006) |
